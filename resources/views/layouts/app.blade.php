...
<body class="antialiased bg-gray-100" x-data="{ 
    ...
    @php
        ...
        $breadcrumbs = [
            ...
            'payment-proofs.index' => ['title' => 'Soportes de Pago', 'parent' => null],
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="sidebar-gradient min-h-screen flex flex-col fixed left-0 top-0 z-40 transition-all duration-300"
            ...
            <nav class="flex-1 p-3 space-y-1 overflow-y-auto">
                ...
                @if($isAdmin || ($user && $user->hasPermission('soportes_pago.ver')))
                <a href="{{ route('payment-proofs.index') }}" 
                   class="flex items-center px-3 py-3 rounded-lg {{ str_starts_with($currentRoute, 'payment-proofs.') ? 'bg-gradient-to-r from-primary-500/20 to-secondary-500/20 text-white border-l-4 border-primary-500' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }} transition-all group"
                   :class="sidebarOpen ? 'space-x-3' : 'justify-center'"
                   :title="!sidebarOpen ? 'Soportes de Pago' : ''">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    <span x-show="sidebarOpen" x-cloak>Soportes de Pago</span>
                </a>
                @endif
