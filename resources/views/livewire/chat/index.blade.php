<div class="h-[calc(100vh-5rem)] flex flex-col overflow-hidden" 
    ...
    @else
        ...
        <div class="flex-1 flex overflow-hidden">
            <!-- Left Column -->
            <div class="w-72 min-w-[270px] flex-shrink-0 bg-white border-r border-gray-200 flex flex-col">
                <!-- Search & Filter -->
            ...
            <div class="flex-1 min-w-0 flex flex-col bg-[#efeae2]" style="background-image: url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%23d5d0c8\' fill-opacity=\'0.4\'%3E%3Cpath d=\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E');">
                @if($selectedContactId && $this->selectedContact)
                    ...
                    <div class="bg-[#f0f2f5] px-4 py-2 border-b border-gray-200 flex items-center gap-3">
                        ...
                        <!-- Sync Conversation Button -->
                        <button wire:click="syncConversation" 
                                wire:loading.attr="disabled"
                                wire:target="syncConversation"
                                class="p-2 text-gray-500 hover:text-green-600 hover:bg-gray-200 rounded-full transition-colors disabled:opacity-50" 
                                title="Sincronizar mensajes de esta conversación desde WhatsApp">
                            <svg wire:loading.remove wire:target="syncConversation" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                            </svg>
                            <svg wire:loading wire:target="syncConversation" class="w-5 h-5 animate-spin text-green-600" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                        </button>
                    ...
                    {{-- LID Info Banner (no bloquea envío) --}}
                    @if($this->selectedContact->needsNumberResolution())
                        <div class="bg-amber-50 border-b border-amber-200 px-4 py-2 flex items-center gap-3">
                            ...
                            <p class="text-xs text-amber-700 flex-1">
                                Contacto sin número real asignado. Puedes continuar la conversación normalmente. Si conoces el número, asígnalo para mejor seguimiento.
                            </p>
                            <button wire:click="openLidModal" class="px-2.5 py-1 bg-amber-500 text-white text-xs font-medium rounded-lg hover:bg-amber-600 transition-colors flex-shrink-0">
                                Asignar número
                            </button>
                        </div>
                    @endif
                    ...
                    <div class="flex-1 overflow-y-auto p-4 space-y-1" id="messages-container"
                        ...
                        @foreach($this->messages as $message)
                            @php
                                ...
                                $showDateSeparator = $messageDate && $messageDate !== $previousDate;
                                $previousDate = $messageDate;
                            @endphp

                            @if($showDateSeparator)
                                <div class="flex items-center justify-center my-3" wire:key="date-sep-{{ $messageDate }}-{{ $message->id }}">
                                    <div class="bg-white px-4 py-1.5 rounded-lg shadow-sm text-xs text-gray-600 font-medium">
                                        @if($message->sent_at->isToday())
                                            Hoy
                                        @elseif($message->sent_at->isYesterday())
                                            Ayer
                                        @else
                                            {{ $message->sent_at->translatedFormat('l, j \\d\\e F \\d\\e Y') }}
                                        @endif
                                    </div>
                                </div>
                            @endif
                                ...
                                <div class="max-w-[70%] {{ $message->direction === 'outgoing' ? 'bg-[#d9fdd3]' : 'bg-white' }} rounded-lg px-3 py-1.5 shadow-sm relative min-w-[60px]"
                                    ...
                                    @elseif($message->type === 'call')
                                        <div class="flex items-center gap-2 text-gray-500 py-1">
                                            ...
                                            <span class="text-sm">📞 Llamada</span>
                                        </div>
                                    @else
                                    ...
                                    <div class="flex items-center justify-end gap-1 {{ in_array($message->type, ['text']) && $message->content ? 'absolute bottom-1 right-2' : 'mt-1' }}">
                                        ...
                                        @if($message->direction === 'outgoing')
                                            ...
                                            @elseif($message->status === 'failed')
                                                @php $sendError = $message->metadata['send_error'] ?? null; @endphp
                                                <button wire:click="retryMessage({{ $message->id }})" class="text-red-500 hover:text-red-700" title="{{ $sendError ? $sendError . ' — Clic para reintentar' : 'Error al enviar — Clic para reintentar' }}">
